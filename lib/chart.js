...
function makeGroups(svg) {
  var parent = svg.append('g').attr('class', 'fan-parent')
    , groups = {
        ...
        borders: parent.append('g').attr('class', 'fan-borders'),
...
Chart.prototype = {
  clear: function () {
    ...
    setTimeout(this.resize.bind(this), 5)
  },
  attach: function (el) {
    this.el = el
  ...
  resize: function () {
    var rect = this.el.getBoundingClientRect()
      , ratio = (this.c.width / this.c.height)
      , height = rect.height >= (this.c.minHeight || 100) ? rect.height : this.c.minHeight
      , width =  ratio * height
      , zoom = width / this.c.width
    if (!rect.height || !rect.width) return
    this.svg.attr('width', parseInt(width, 10))
      .attr('height', parseInt(height, 10))
    this.groups.parent.attr('transform', 'scale(' + zoom + ')')
    console.log('resize', width, height, zoom)
  },
  ...
  makePath: function (node, start, extend) {
    var path = arcPath(this.c.center, node.gen, node.pos, {
      sweep: this.c.sweep,
      offset: 0,
      extend: extend,
  ...
  formatText: function (node, text, lines, rotate, size) {
    var center = arcCenter(this.c.center, node.gen, node.pos, {
      sweep: this.c.sweep,
      offset: 0,
      width: this.c.ringWidth,
    ...
    size = size || this.c.ringWidth / 7;
    if (rotate) {
      transform += ' rotate(' + (center.angle * 180 / Math.PI) + ')';
    }
  ...
  makeText: function (node, lines) {
    var text
      ...
      , back;
    if (this.c.svgtips) {
      var tgroup = this.groups.text.insert('g')
        .attr('class', 'hovertext');
      if (node.photolink) {
        tgroup = tgroup.insert('a', 'a')
          .attr('target', '_blank')
          .attr('xlink:href', node.photolink);
      }
      tgroup.insert('path', 'path')
        .attr('d', this.makePath(node));
      back = tgroup.insert('text');
      this.formatText(node, back, lines, false, 25);
      back.attr('class', 'background');
      text = tgroup.insert('text');
      text.attr('class', 'foreground');
    } else {
      text = this.groups.text.insert('text');
    }
    this.formatText(node, text, lines, !this.c.svgtips, this.c.svgtips && 25);
    return text;
  },
  node: function (node, link, text, photo) {
    ...
    if (!node.el) {
      ...
      if (this.c.indicators) {
        node.indicators = this.makeIndicators(node);
        node.border = this.groups.borders.insert('path', 'path')
          .attr('class', 'arc border');
        node.border.attr('d', node.el.attr('d'));
      }
    }
    if (!node.father) {
      node.father = {
  ...
  makeIndicator: function (node, start, length) {
    var ind = this.groups.colors.insert('path', 'path')
                .attr('class', 'arc indicator');
    ind.attr('d', this.makePath(node, start, length));
    return ind;
  },
  makeIndicators: function (node) {
    var inds = this.c.indicators
      , start = 0
      , ret = []
      , len
      , ind;
    if ('object' !== typeof inds) {
      return [this.makeIndicator(node, 0, inds === true ? .2 : inds)]
    }
    for (var i=0; i<inds.length; i++) {
      ...
      ret.push(this.makeIndicator(node, start, len));
      start += len;
    }
    return ret;
  },
