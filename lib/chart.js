...
var Chart = function (options) {
  ...
  if (options.el) this.attach(options.el);
};
...
Chart.prototype = {
  ...
  makePath: function (node) {
    var path = arcPath(this.options.center, node.gen, node.pos, {
      sweep: Math.PI*5/4,
      offset: 0,
      width: this.options.ringWidth,
      doubleWidth: this.options.doubleWidth
    });
    return makePath(path) + 'z';
  },
  node: function (node, person, name, scope) {
    var self = this;
    if (!node.el) {
      node.el = this.groups.paths.insert('path', 'path')
        .attr('class', 'arc person');
      node.el.attr('d', this.makePath(node));
      // node.el.style('fill', person.color || this.options.color);
    }
    if (!node.father) {
      node.father = {
        gen: node.gen + 1,
